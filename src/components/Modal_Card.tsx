            ...
            <Dialog className='bg-black items-center h-dvh p-4' open={openModalCard} handler={() => handleOpenModalCard}>
                <div className='text-white border w-full border-orange_400 border-dashed rounded-xl justify-center text-center mx-auto'>
                    ...
                    <DialogFooter className='justify-center gap-8'>
                        {page_link && <a href={page_link} target="_blank" rel="noopener"><Button tittle="Página" /></a>}
                        <a className="outline-none" href={git_link} target="_blank" rel="noopener"><Button tittle="Repositório" /></a>
                        <a onClick={handleOpenModalCard}><Button tittle='Sair'/></a>
                    </DialogFooter>
                </div>
            </Dialog>
        </div>
